...
{
  static Name strategyName("/localhost/nfd/strategy/firewall/%FD%01");
  return strategyName;
}

int
FirewallStrategy::checkInterest(const Interest& interest)
{
  ndn::Name BannedName(BannedString);
  ndn::InterestFilter BannedFilter(BannedName);	
  if(BannedFilter.doesMatch(interest.getName())==1)
  {
    NFD_LOG_DEBUG("Firewall WARNING: Match " << interest.getName().toUri() << "from <" << BannedName);
    return -1;
  }
  else
  {
